...
- **分段对比** —— 启 / 禁用本地分段时间对比、PB 与手动加载采样路径、多关对比的初始项目,以及各项检测 / 结算详细参数。多关运行中项目仅当前局内自动升级(`Any%` ⊃ `Steam%` ⊃ `Dark%` ⊃ `Aztec%`),不会改写已保存设置;若所选项目完全没有数据,则回退到有数据的最小项目(仅当前局内)。排行榜显示 / 隐藏键位于 **常规 → 按键绑定**;排行榜外观设置已移到下面的 **排行榜** 标签页。
...
- **显示的资料** —— 页面底部提供每个 subsegment 资料的开关:**PB** 与加载目录下的每个顶层文件夹。只有勾选的资料会出现在排行榜中。过滤排序后仍按 `MaxLeaderboardEntries` 截断到前 N 项。
