...
- **按键绑定** —— 重置 / 重试 / 设置键 / 排行榜显示切换键。排行榜开关键用于显示 / 隐藏共享的排行榜 HUD,无论它显示的是分段对比参考还是标记 feed。改绑方法:点击对应项,然后按下目标键。纯修饰键(Shift/Ctrl/Alt/Cmd)按下会被忽略。鼠标侧键(`Mouse3`–`Mouse6`)也可绑定;鼠标左 / 右键仍保留为普通 UI 操作,不可绑定。
...
- **内容** —— 选择共享排行榜 HUD 显示什么:**分段对比**(参考时间比较)或**标记**(当前关卡的标记 feed)。显示 / 隐藏键与下面的外观设置对两种模式都生效。
- **面板** —— 排行榜字号、横向偏移、纵向偏移(相对自动垂直居中)。
...
- **标记** —— 启 / 禁用标记模块,切换**标记编辑模式**(同时开启游戏内可视化与醒目的 HUD 提示),并按 **主梦境**(内置关卡)、**额外梦境**(editor pick 关卡)、**工坊** 浏览关卡;每个关卡按钮进入该关卡的标记页。
